...
目前專案中的完整 CIFAR-10 訓練腳本位於 [train_split.py](../python/train_split.py)，也使用同一個 `.so`。PyTorch 對照 baseline 位於 [train_split_torch_baseline.py](../python/train_split_torch_baseline.py)，兩者共用 `train_config.py`、`cifar10_data.py` 與 `model_init.py` 來維持相同資料切分、初始權重與 Momentum SGD 條件。
