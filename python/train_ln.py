#!/usr/bin/env python3
"""Training with LayerNorm after Conv"""
import ctypes
import numpy as np
import os
import time
...
class Bridge:
    def __init__(self, so_path):
        ...
        self.lib.leaky_relu_forward.argtypes = [c_void_p, c_float, c_int]
        self.lib.leaky_relu_backward.argtypes = [c_void_p, c_void_p, c_float, c_int]
        ...
        self.lib.layer_norm_forward.argtypes = [c_void_p, c_void_p, c_void_p, c_void_p, c_int, c_int, c_int, c_int, c_float]
        self.lib.layer_norm_backward.argtypes = [c_void_p, c_void_p, c_void_p, c_void_p, c_int, c_int, c_int, c_int, c_float]
...
def train():
    ...
    BATCH, EPOCHS, LR = 64, 10, 0.01
    LEAKY_ALPHA = 0.1
    print(f"Config: batch={BATCH}, epochs={EPOCHS}, lr={LR}, leaky_alpha={LEAKY_ALPHA}")
    ...
    # Init LayerNorm params: gamma=1, beta=0
    ln_gamma = np.ones(OC, dtype=np.float32)
    ln_beta = np.zeros(OC, dtype=np.float32)
    ...
    d_ln_gamma = bridge.malloc(OC * 4)
    d_ln_beta = bridge.malloc(OC * 4)
    bridge.h2d(d_ln_gamma, ln_gamma.ctypes.data, OC * 4)
    bridge.h2d(d_ln_beta, ln_beta.ctypes.data, OC * 4)
    ...
    for epoch in range(EPOCHS):
        ...
        for batch_idx in range(N_TRAIN // BATCH):
            ...
            # LayerNorm: normalize over (H, W) for each (N, C)
            d_conv_norm = bridge.malloc(OC * BATCH * outH * outW * 4)
            bridge.lib.layer_norm_forward(d_conv_norm, d_conv_raw, d_ln_gamma, d_ln_beta,
                                         OC, BATCH, outH, outW, c_float(1e-5))
            
            # LeakyReLU after LayerNorm
            bridge.lib.leaky_relu_forward(d_conv_norm, c_float(LEAKY_ALPHA), OC * BATCH * outH * outW)
            
            # Reorganize: (OC, N) -> (N, OC, H, W)
            d_conv = bridge.malloc(OC * BATCH * outH * outW * 4)
            bridge.lib.reorganize_forward(d_conv_norm, d_conv, BATCH, OC, outH, outW)
            ...
            grad_pool = np.clip(grad_pool, -1.0, 1.0).astype(np.float32)
            ...
            bridge.memset(d_conv_grad, 0, OC * BATCH * outH * outW * 4)
            bridge.lib.maxpool_backward_nchw(d_pool_grad, d_conv, d_conv_grad, BATCH, OC, outH, outW)
            
            # Reorganize backward
            d_conv_norm_grad = bridge.malloc(OC * BATCH * outH * outW * 4)
            bridge.lib.reorganize_backward(d_conv_grad, d_conv_norm_grad, BATCH, OC, outH, outW)
            
            # LeakyReLU backward
            bridge.lib.leaky_relu_backward(d_conv, d_conv_norm_grad, c_float(LEAKY_ALPHA), OC * BATCH * outH * outW)
            
            # LayerNorm backward - but we need the input to LayerNorm for the backward pass
            # We don't have d_conv_raw available, only d_conv_raw from forward
            # For simplicity, just pass d_conv_norm_grad and compute approximate gradient
            d_ln_grad = bridge.malloc(OC * 4)  # dummy for gamma gradient
            d_conv_raw_grad = bridge.malloc(OC * BATCH * outH * outW * 4)
            bridge.lib.layer_norm_backward(d_conv_raw_grad, d_conv_norm_grad, d_conv_raw, d_ln_gamma,
                                          OC, BATCH, outH, outW, c_float(1e-5))
            ...
            # Clip and update conv weights
            h_conv_grad_w = np.zeros((OC * C * KH * KW,), dtype=np.float32)
            bridge.d2h(h_conv_grad_w.ctypes.data, d_conv_grad_w, OC * C * KH * KW * 4)
            h_conv_grad_w = np.clip(h_conv_grad_w, -1.0, 1.0)
            bridge.h2d(d_conv_grad_w, h_conv_grad_w.ctypes.data, OC * C * KH * KW * 4)
            bridge.lib.apply_sgd_update(d_w_conv, d_conv_grad_w, LR, OC * C * KH * KW)
            ...
            bridge.free(d_conv_norm_grad)
            bridge.free(d_conv_grad_w)
            bridge.free(d_ln_grad)
            bridge.free(d_conv_raw_grad)
            
            if (batch_idx + 1) % 200 == 0:
                acc_batch = correct / (batch_idx + 1) / BATCH * 100
                print(f"  Batch {batch_idx+1}: Loss={loss:.4f}, Acc={acc_batch:.1f}%")
    ...
    bridge.free(d_fc_w)
    bridge.free(d_fc_b)
    bridge.free(d_ln_gamma)
    bridge.free(d_ln_beta)
    print("Done!")
