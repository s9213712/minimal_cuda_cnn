...
# Find where max is
max_val = h_conv[0, 0].max()
max_loc = np.where(h_conv[0, 0] == max_val)
print("Max in ch0:", max_val, "at row", max_loc[0], "col", max_loc[1])
...
# Check pool output for channel 0
print("\nPool ch0 max:", h_pool[0, 0].max())
pool_max_loc = np.where(h_pool[0, 0] == h_pool[0, 0].max())
print("Pool max at row", pool_max_loc[0], "col", pool_max_loc[1])
