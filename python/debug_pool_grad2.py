#!/usr/bin/env python3
"""Check pool backward gradient for actual data"""
import ctypes
import numpy as np
...
# Check conv values at first spatial location
h_conv = np.zeros((BATCH, OC, outH, outW), dtype=np.float32)
lib.gpu_memcpy_d2h(h_conv.ctypes.data, d_conv, BATCH * OC * outH * outW * 4)

# Find max in first 2x2 window for batch 0, channel 0
window = h_conv[0, 0, 0:2, 0:2]
print("First 2x2 window (batch 0, ch 0):")
print(window)
print(f"Max location in window: row={window.argmax()//2}, col={window.argmax()%2}")
print(f"Max value: {window.max():.6f}")

# Pool output at (0, 0, 0, 0) should be max of first 2x2 window
h_pool = np.zeros((BATCH, OC, 15, 15), dtype=np.float32)
lib.gpu_memcpy_d2h(h_pool.ctypes.data, d_pool, BATCH * OC * 15 * 15 * 4)
print(f"\nPool output at (0,0,0,0): {h_pool[0,0,0,0]:.6f}")

# Now test backward with ones gradient
grad_out_data = np.ones((BATCH, OC, 15, 15), dtype=np.float32)
d_pool_grad = lib.gpu_malloc(OC * BATCH * 15 * 15 * 4)
lib.gpu_memcpy_h2d(d_pool_grad, grad_out_data.ctypes.data, OC * BATCH * 15 * 15 * 4)
...
print(f"\nConv grad at (0,0,0,0): {h_conv_grad[0,0,0,0]:.6f}")
print(f"Conv grad at (0,0,0,1): {h_conv_grad[0,0,0,1]:.6f}")
print(f"Conv grad at (0,0,1,0): {h_conv_grad[0,0,1,0]:.6f}")
print(f"Conv grad at (0,0,1,1): {h_conv_grad[0,0,1,1]:.6f}")
print(f"Sum of conv grad in first 2x2: {h_conv_grad[0,0,0:2,0:2].sum():.6f}")
