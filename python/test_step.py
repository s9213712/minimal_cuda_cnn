...
# Verify weights
hw1 = np.zeros(C1_OUT * C1_IN * KH1 * KW1, dtype=np.float32)
hw2 = np.zeros(C2_OUT * C2_IN * KH2 * KW2, dtype=np.float32)
hfw = np.zeros(10 * FC_IN, dtype=np.float32)
lib.gpu_memcpy_d2h(hw1.ctypes.data, dw1, C1_OUT * C1_IN * KH1 * KW1 * 4)
lib.gpu_memcpy_d2h(hw2.ctypes.data, dw2, C2_OUT * C2_IN * KH2 * KW2 * 4)
lib.gpu_memcpy_d2h(hfw.ctypes.data, dfw, 10 * FC_IN * 4)
print(f"Verify dw1: min={hw1.min():.6f}, max={hw1.max():.6f}")
print(f"Verify dw2: min={hw2.min():.6f}, max={hw2.max():.6f}")
print(f"Verify dfw: min={hfw.min():.6f}, max={hfw.max():.6f}")
