#!/usr/bin/env python3
"""Track first 10 batches in detail"""
import ctypes
import numpy as np
...
for batch_idx in range(10):
    ...
    print(f"Batch {batch_idx}: h_out min={h_out.min():.4f} max={h_out.max():.4f} | probs min={probs.min():.6f} max={probs.max():.6f} | loss={loss:.4f} | h_p2 min={h_p2.min():.4f} max={h_p2.max():.4f}", flush=True)
    ...
    print(f"  grad_fc_w_c: min={grad_fc_w_c.min():.4f} max={grad_fc_w_c.max():.4f} nan={np.isnan(grad_fc_w_c).any()}", flush=True)
