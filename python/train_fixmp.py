#!/usr/bin/env python3
"""Training with correct MaxPool (stores argmax)"""
import ctypes
import numpy as np
import os
import time
...
class Bridge:
    def __init__(self, so_path):
        ...
        self.lib.maxpool_forward_store.argtypes = [c_void_p, c_void_p, c_void_p, c_int, c_int, c_int, c_int]
        self.lib.maxpool_backward_use_idx.argtypes = [c_void_p, c_void_p, c_void_p, c_int, c_int, c_int, c_int]
...
def train():
    ...
    print("=== Training with Correct MaxPool ===")
    ...
    for epoch in range(EPOCHS):
        ...
        for batch_idx in range(N_TRAIN // BATCH):
            ...
            bridge.lib.leaky_relu_forward(d_conv_raw, c_float(LEAKY_ALPHA), OC * BATCH * outH * outW)
            ...
            # Pool with stored max indices
            d_pool = bridge.malloc(OC * BATCH * 15 * 15 * 4)
            d_max_idx = bridge.malloc(OC * BATCH * 15 * 15 * 4)  # Store indices
            bridge.lib.maxpool_forward_store(d_pool, d_conv, d_max_idx, BATCH, OC, outH, outW)
            ...
            # Pool backward using stored max indices
            d_conv_grad = bridge.malloc(OC * BATCH * outH * outW * 4)
            bridge.memset(d_conv_grad, 0, OC * BATCH * outH * outW * 4)
            bridge.lib.maxpool_backward_use_idx(d_pool_grad, d_max_idx, d_conv_grad, BATCH, OC, outH, outW)
            ...
            # LeakyReLU backward
            bridge.lib.leaky_relu_backward(d_conv, d_conv_raw_grad, c_float(LEAKY_ALPHA), OC * BATCH * outH * outW)
