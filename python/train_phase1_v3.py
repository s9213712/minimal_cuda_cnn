#!/usr/bin/env python3
"""Phase 1: mini-AlexNet with 2 Conv layers - CORRECTED
Key fix: Compute ALL gradients BEFORE applying any weight updates
"""
import ctypes
import numpy as np
import time
...
for epoch in range(EPOCHS):
    ...
    for batch_idx in range(NBATCHES):
        ...
        print(f"DEBUG batch{batch_idx}: h_pool2 min={h_pool2.min():.4f} max={h_pool2.max():.4f}")
        ...
        # ============ BACKWARD - COMPUTE GRADIENTS FIRST ============
        labels_onehot = np.zeros((BATCH, 10), dtype=np.float32)
        labels_onehot[np.arange(BATCH), y] = 1.0
        d_loss = probs - labels_onehot  # (64, 10)
        
        # FC grad - using h_pool2 (frozen from forward)
        grad_fc_w = d_loss.T @ h_pool2  # (10, 2304) - use h_pool2 NOT updated d_fc_w
        grad_fc_w_clipped = np.clip(grad_fc_w, -1.0, 1.0).flatten().astype(np.float32)
        
        # Pool2 grad - using ORIGINAL fc_w (not updated yet)
        fc_w_reshaped = fc_w.reshape(10, FC_IN)
        grad_pool2 = d_loss @ fc_w_reshaped  # (64, 2304) - using ORIGINAL fc_w
        ...
        # Conv2 weight grad - SKIP for now to isolate crash
        ...
        # h_w_conv2 = np.zeros(C2_OUT * C2_IN * KH2 * KW2, dtype=np.float32)
        # lib.gpu_memcpy_d2h(h_w_conv2.ctypes.data, d_w_conv2, C2_OUT * C2_IN * KH2 * KW2 * 4)
        # h_w_conv2_clipped = np.clip(h_w_conv2, -1.0, 1.0).astype(np.float32)
