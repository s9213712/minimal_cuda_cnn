...
h_raw = np.zeros((OC, BATCH * outH * outW), dtype=np.float32)
lib.gpu_memcpy_d2h(h_raw.ctypes.data, d_conv_raw, OC * BATCH * outH * outW * 4)
print("Raw conv (OC, N):")
print("  min:", h_raw.min(), "max:", h_raw.max())
print("  any inf:", np.isinf(h_raw).any())
print("  any nan:", np.isnan(h_raw).any())
...
print("\nReorganized conv (N, OC, H, W):")
print("  min:", h_conv.min(), "max:", h_conv.max())
print("  any inf:", np.isinf(h_conv).any())
print("  any nan:", np.isnan(h_conv).any())
print("  n=0, c=0, min:", h_conv[0, 0].min(), "max:", h_conv[0, 0].max())
