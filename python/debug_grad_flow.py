...
lib.apply_relu_backward.argtypes = [c_void_p, c_void_p, c_int]
...
print(f"FC grad: min={grad_fc_w.min():.6f}, max={grad_fc_w.max():.6f}")

d_fc_grad_w = lib.gpu_malloc(10 * FC_IN * 4)
lib.gpu_memcpy_h2d(d_fc_grad_w, grad_fc_w.ctypes.data, 10 * FC_IN * 4)
lib.apply_sgd_update(d_fc_w, d_fc_grad_w, 0.1, 10 * FC_IN)

# Pool grad
fc_w_reshaped = fc_w.reshape(10, FC_IN)
grad_pool = d_loss @ fc_w_reshaped
grad_pool = np.clip(grad_pool, -1.0, 1.0).astype(np.float32)
print(f"Pool grad: min={grad_pool.min():.6f}, max={grad_pool.max():.6f}")
...
print(f"Conv grad after pool: min={h_conv_grad.min():.6f}, max={h_conv_grad.max():.6f}")

# ReLU backward
lib.apply_relu_backward(d_conv, d_conv_grad, OC * BATCH * outH * outW)
lib.gpu_memcpy_d2h(h_conv_grad.ctypes.data, d_conv_grad, BATCH * OC * outH * outW * 4)
print(f"Conv grad after relu: min={h_conv_grad.min():.6f}, max={h_conv_grad.max():.6f}")
...
print(f"Conv raw grad: min={h_conv_raw_grad.min():.6f}, max={h_conv_raw_grad.max():.6f}")
...
print(f"Conv weight grad: min={h_conv_grad_w.min():.6f}, max={h_conv_grad_w.max():.6f}")
print(f"Conv weight grad has NaN: {np.isnan(h_conv_grad_w).any()}")
print(f"Conv weight grad has Inf: {np.isinf(h_conv_grad_w).any()}")
