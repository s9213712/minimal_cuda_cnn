#!/usr/bin/env python3
"""Exact copy of train_phase1_v3 backward section"""
import ctypes
import numpy as np
...
# FORWARD (exactly like test_step.py)
...
# BACKWARD (exactly like train_phase1_v3.py)
labels_onehot = np.zeros((BATCH, 10), dtype=np.float32)
labels_onehot[np.arange(BATCH), y] = 1.0
d_loss = (h_out - h_out.max(axis=1, keepdims=True))
exp_o = np.exp(d_loss)
...
print(f"fc_w after: min={fc_w.min():.4f} max={fc_w.max():.4f}")
...
print(f"gp2: min={gp2.min():.4f} max={gp2.max():.4f}")
print(f"gp2_c: min={gp2_c.min():.4f} max={gp2_c.max():.4f}")

# Now FREE backward-related allocations
lib.gpu_free(dgfw)
lib.gpu_free(dp2g)
lib.gpu_free(dc2g)

# Now re-run forward to see if it's corrupted
lib.gpu_memcpy_h2d(dfi, h_pool2.ctypes.data, BATCH * FC_IN * 4)
lib.dense_forward(dfi, dfw, dfb, dfo, BATCH, FC_IN, 10)
h_out2 = np.zeros((BATCH, 10), dtype=np.float32)
lib.gpu_memcpy_d2h(h_out2.ctypes.data, dfo, BATCH * 10 * 4)
print(f"h_out2 (second forward): min={h_out2.min():.4f} max={h_out2.max():.4f}")
...
lib.gpu_free(dfo); lib.gpu_free(dw1); lib.gpu_free(dw2); lib.gpu_free(dfw); lib.gpu_free(dfb)
