#!/usr/bin/env python3
"""Isolate NaN issue - track values through training loop"""
import ctypes
import numpy as np
...
with open(os.path.join(data_root, "data_batch_1"), "rb") as f:
    ...
    x_all = batch[b"data"][:256].astype(np.float32) / 255.0
    y_all = np.array(batch[b"labels"][:256])
...
for batch_idx in range(4):
    ...
    print(f"Batch {batch_idx}: h_out min={h_out.min():.4f} max={h_out.max():.4f}")
    print(f"           probs min={probs.min():.6f} max={probs.max():.6f}")
    ...
    print(f"           fc_w after update min={fc_w.min():.4f} max={fc_w.max():.4f}")
    
    # Pool2 backward (NO conv2 backward)
