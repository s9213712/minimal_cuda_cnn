...
# Load data_batch_1 for training (8000 train, 2000 val)
print("Loading data_batch_1...")
...
val_idx = indices[8000:10000]
x_train = x_all[train_idx]
y_train = y_all[train_idx]
x_val = x_all[val_idx]
y_val = y_all[val_idx]

# Load official test batch
...
for epoch in range(EPOCHS):
    ...
    for batch_idx in range(NBATCHES):
        ...
        # FIXED conv_backward: now computes d_pool1_grad too
        d_w_conv2_grad = lib.gpu_malloc(C2_OUT*C2_IN*KH2*KW2*4)
        d_pool1_grad = lib.gpu_malloc(C2_IN*BATCH*poolH1*poolW1*4)  # NEW: input gradient
        ...
        # === CONV1 BACKWARD ===
        # pool1 backward: d_pool1_grad -> d_conv1_grad_raw
        ...
        # conv1 backward: compute d_x_grad (but don't use it for now)
        d_w_conv1_grad = lib.gpu_malloc(C1_OUT*C1_IN*KH1*KW1*4)
        d_x_grad = lib.gpu_malloc(BATCH*C1_IN*H*W*4)  # unused input gradient
...
print("\nDone! conv_backward FIXED - both Conv1 and Conv2 updating")
