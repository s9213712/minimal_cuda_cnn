#!/usr/bin/env python3
"""Phase 1: mini-AlexNet with 2 Conv layers - TRACKING VERSION"""
import ctypes
import numpy as np
import time
...
for epoch in range(EPOCHS):
    ...
    for batch_idx in range(NBATCHES):
        ...
        if np.isnan(grad_fc_w_clipped).any():
            print(f"  Batch {batch_idx}: grad_fc_w has NaN!")
        ...
        if np.isnan(h_fc_w).any():
            print(f"  Batch {batch_idx}: fc_w has NaN after update!")
        fc_w = h_fc_w.copy()
        ...
        if (batch_idx + 1) % 5 == 0 or batch_idx == 0:
            h_dw2g = np.zeros(C2_OUT * C2_IN * KH2 * KW2, dtype=np.float32)
            lib.gpu_memcpy_d2h(h_dw2g.ctypes.data, d_w_conv2_grad, C2_OUT * C2_IN * KH2 * KW2 * 4)
            print(f"  Batch {batch_idx}: dw2_grad min={h_dw2g.min():.4f}, max={h_dw2g.max():.4f}")
        ...
        if np.isnan(h_w_conv2).any():
            print(f"  Batch {batch_idx}: w_conv2 has NaN after update!")
        ...
        if (batch_idx + 1) % 100 == 0 or batch_idx == 0:
            print(f"  Epoch {epoch+1} Batch {batch_idx+1}/{NBATCHES}: Loss={loss:.4f}, Acc={correct/(batch_idx+1)/BATCH*100:.1f}%")
