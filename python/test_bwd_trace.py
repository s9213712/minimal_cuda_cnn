#!/usr/bin/env python3
"""Trace backward pass for NaN"""
import ctypes
import numpy as np
from ctypes import c_void_p, c_float, c_int
import os
import pickle

so = os.path.join(os.path.dirname(__file__), "..", "cpp", "libminimal_cuda_cnn.so")
lib = ctypes.CDLL(so)
lib.gpu_malloc.restype = c_void_p
lib.gpu_malloc.argtypes = [ctypes.c_size_t]
lib.gpu_free.argtypes = [c_void_p]
lib.gpu_memcpy_h2d.argtypes = [c_void_p, c_void_p, ctypes.c_size_t]
lib.gpu_memcpy_d2h.argtypes = [c_void_p, c_void_p, ctypes.c_size_t]
lib.gpu_memset.argtypes = [c_void_p, c_int, ctypes.c_size_t]
lib.im2col_forward.argtypes = [c_void_p, c_void_p, c_int, c_int, c_int, c_int, c_int, c_int, c_int, c_int]
lib.gemm_forward.argtypes = [c_void_p, c_void_p, c_void_p, c_int, c_int, c_int]
lib.leaky_relu_forward.argtypes = [c_void_p, c_float, c_int]
lib.reorganize_forward.argtypes = [c_void_p, c_void_p, c_int, c_int, c_int, c_int]
lib.maxpool_forward_store.argtypes = [c_void_p, c_void_p, c_void_p, c_int, c_int, c_int, c_int]
lib.maxpool_backward_use_idx.argtypes = [c_void_p, c_void_p, c_void_p, c_int, c_int, c_int, c_int]
lib.dense_forward.argtypes = [c_void_p, c_void_p, c_void_p, c_void_p, c_int, c_int, c_int]
lib.apply_sgd_update.argtypes = [c_void_p, c_void_p, c_float, c_int]
lib.conv_backward.argtypes = [c_void_p, c_void_p, c_void_p, c_void_p, c_int, c_int, c_int, c_int, c_int, c_int, c_int, c_int]

BATCH = 64
LR = 0.01
C1_IN, C1_OUT = 3, 32
H, W = 32, 32
KH1, KW1 = 3, 3
outH1, outW1 = 30, 30
poolH1, poolW1 = 15, 15
C2_IN, C2_OUT = 32, 64
KH2, KW2 = 3, 3
outH2, outW2 = 13, 13
poolH2, poolW2 = 6, 6
FC_IN = 2304

np.random.seed(42)
w1 = np.random.randn(C1_OUT*C1_IN*KH1*KW1).astype(np.float32) * 0.05
w2 = np.random.randn(C2_OUT*C2_IN*KH2*KW2).astype(np.float32) * 0.05
fc_w = np.random.randn(10*FC_IN).astype(np.float32) * 0.05
fc_b = np.zeros(10, dtype=np.float32)

dw1 = lib.gpu_malloc(C1_OUT*C1_IN*KH1*KW1*4)
dw2 = lib.gpu_malloc(C2_OUT*C2_IN*KH2*KW2*4)
dfw = lib.gpu_malloc(10*FC_IN*4)
dfb = lib.gpu_malloc(40)
lib.gpu_memcpy_h2d(dw1, w1.ctypes.data, C1_OUT*C1_IN*KH1*KW1*4)
lib.gpu_memcpy_h2d(dw2, w2.ctypes.data, C2_OUT*C2_IN*KH2*KW2*4)
lib.gpu_memcpy_h2d(dfw, fc_w.ctypes.data, 10*FC_IN*4)

data_root = os.path.join(os.path.dirname(__file__), "..", "data", "cifar-10-batches-py")
with open(os.path.join(data_root, "data_batch_1"), "rb") as f:
    batch = pickle.load(f, encoding="bytes")
    x = batch[b"data"][:BATCH].astype(np.float32) / 255.0
    y = np.array(batch[b"labels"][:BATCH])
    x = x.reshape(-1, 3, 32, 32)

dx = lib.gpu_malloc(BATCH*C1_IN*H*W*4)
lib.gpu_memcpy_h2d(dx, x.ctypes.data, BATCH*C1_IN*H*W*4)

dc1r = lib.gpu_malloc(C1_OUT*BATCH*outH1*outW1*4)
dc1 = lib.gpu_malloc(C1_OUT*BATCH*outH1*outW1*4)
dp1 = lib.gpu_malloc(C1_OUT*BATCH*poolH1*poolW1*4)
dmi1 = lib.gpu_malloc(C1_OUT*BATCH*poolH1*poolW1*4)
dc1_col = lib.gpu_malloc(C1_IN*KH1*KW1*BATCH*outH1*outW1*4)
lib.im2col_forward(dx, dc1_col, BATCH, C1_IN, H, W, KH1, KW1, outH1, outW1)
lib.gemm_forward(dw1, dc1_col, dc1r, C1_OUT, BATCH*outH1*outW1, C1_IN*KH1*KW1)
lib.leaky_relu_forward(dc1r, c_float(0.1), C1_OUT*BATCH*outH1*outW1)
lib.reorganize_forward(dc1r, dc1, BATCH, C1_OUT, outH1, outW1)
lib.maxpool_forward_store(dp1, dc1, dmi1, BATCH, C1_OUT, outH1, outW1)

dc2r = lib.gpu_malloc(C2_OUT*BATCH*outH2*outW2*4)
dc2 = lib.gpu_malloc(C2_OUT*BATCH*outH2*outW2*4)
dp2 = lib.gpu_malloc(C2_OUT*BATCH*poolH2*poolW2*4)
dmi2 = lib.gpu_malloc(C2_OUT*BATCH*poolH2*poolW2*4)
dc2_col = lib.gpu_malloc(C2_IN*KH2*KW2*BATCH*outH2*outW2*4)
lib.im2col_forward(dp1, dc2_col, BATCH, C2_IN, poolH1, poolW1, KH2, KW2, outH2, outW2)
lib.gemm_forward(dw2, dc2_col, dc2r, C2_OUT, BATCH*outH2*outW2, C2_IN*KH2*KW2)
lib.leaky_relu_forward(dc2r, c_float(0.1), C2_OUT*BATCH*outH2*outW2)
lib.reorganize_forward(dc2r, dc2, BATCH, C2_OUT, outH2, outW2)
lib.maxpool_forward_store(dp2, dc2, dmi2, BATCH, C2_OUT, outH2, outW2)

h_p2 = np.zeros((BATCH, FC_IN), dtype=np.float32)
lib.gpu_memcpy_d2h(h_p2.ctypes.data, dp2, BATCH*FC_IN*4)
dfi = lib.gpu_malloc(BATCH*FC_IN*4)
lib.gpu_memcpy_h2d(dfi, h_p2.ctypes.data, BATCH*FC_IN*4)
dfo = lib.gpu_malloc(BATCH*10*4)
lib.dense_forward(dfi, dfw, dfb, dfo, BATCH, FC_IN, 10)

h_out = np.zeros((BATCH, 10), dtype=np.float32)
lib.gpu_memcpy_d2h(h_out.ctypes.data, dfo, BATCH*10*4)

h_sh = h_out - h_out.max(axis=1, keepdims=True)
exp_o = np.exp(h_sh)
probs = exp_o / exp_o.sum(axis=1, keepdims=True)
loss = -np.mean(np.log(probs[np.arange(BATCH), y] + 1e-10))
print(f"Loss: {loss:.4f}")

# ===== BACKWARD =====
print("\n=== BACKWARD ===")
labels_onehot = np.zeros((BATCH, 10), dtype=np.float32)
labels_onehot[np.arange(BATCH), y] = 1.0
d_loss = probs - labels_onehot
print(f"d_loss: min={d_loss.min():.4f}, max={d_loss.max():.4f}, NaN={np.isnan(d_loss).any()}")

# FC grad
grad_fc_w = d_loss.T @ h_p2
grad_fc_w_clipped = np.clip(grad_fc_w, -1.0, 1.0).flatten().astype(np.float32)
print(f"grad_fc_w_clipped: min={grad_fc_w_clipped.min():.4f}, max={grad_fc_w_clipped.max():.4f}, NaN={np.isnan(grad_fc_w_clipped).any()}")
d_fc_grad_w = lib.gpu_malloc(10*FC_IN*4)
lib.gpu_memcpy_h2d(d_fc_grad_w, grad_fc_w_clipped.ctypes.data, 10*FC_IN*4)
print("FC weight update...")
lib.apply_sgd_update(dfw, d_fc_grad_w, c_float(LR), 10*FC_IN)
print(f"After FC update, dfw: copying back...")
h_dfw = np.zeros(10*FC_IN, dtype=np.float32)
lib.gpu_memcpy_d2h(h_dfw.ctypes.data, dfw, 10*FC_IN*4)
print(f"dfw: min={h_dfw.min():.4f}, max={h_dfw.max():.4f}, NaN={np.isnan(h_dfw).any()}")
# Update CPU fc_w too
fc_w = h_dfw.copy()
fc_w_reshaped = fc_w.reshape(10, FC_IN)

# Pool2 grad
grad_pool2 = d_loss @ fc_w_reshaped
grad_pool2_clipped = np.clip(grad_pool2, -1.0, 1.0).astype(np.float32)
print(f"grad_pool2_clipped: min={grad_pool2_clipped.min():.4f}, max={grad_pool2_clipped.max():.4f}, NaN={np.isnan(grad_pool2_clipped).any()}")

d_pool2_grad = lib.gpu_malloc(C2_OUT*BATCH*poolH2*poolW2*4)
lib.gpu_memcpy_h2d(d_pool2_grad, grad_pool2_clipped.flatten().ctypes.data, C2_OUT*BATCH*poolH2*poolW2*4)

d_conv2_grad = lib.gpu_malloc(C2_OUT*BATCH*outH2*outW2*4)
lib.gpu_memset(d_conv2_grad, 0, C2_OUT*BATCH*outH2*outW2*4)
print("Calling maxpool_backward_use_idx...")
lib.maxpool_backward_use_idx(d_pool2_grad, dmi2, d_conv2_grad, BATCH, C2_OUT, outH2, outW2)
h_c2g = np.zeros(C2_OUT*BATCH*outH2*outW2, dtype=np.float32)
lib.gpu_memcpy_d2h(h_c2g.ctypes.data, d_conv2_grad, C2_OUT*BATCH*outH2*outW2*4)
print(f"d_conv2_grad: min={h_c2g.min():.4f}, max={h_c2g.max():.4f}, NaN={np.isnan(h_c2g).any()}")

# Conv2 weight grad
print("Allocating d_w_conv2_grad...")
d_w_conv2_grad = lib.gpu_malloc(C2_OUT*C2_IN*KH2*KW2*4)
lib.gpu_memset(d_w_conv2_grad, 0, C2_OUT*C2_IN*KH2*KW2*4)
print("Calling conv_backward...")
lib.conv_backward(d_conv2_grad, dp1, dw2, d_w_conv2_grad,
                  BATCH, C2_IN, poolH1, poolW1, KH2, KW2, outH2, outW2, C2_OUT)
h_w2g = np.zeros(C2_OUT*C2_IN*KH2*KW2, dtype=np.float32)
lib.gpu_memcpy_d2h(h_w2g.ctypes.data, d_w_conv2_grad, C2_OUT*C2_IN*KH2*KW2*4)
print(f"d_w_conv2_grad: min={h_w2g.min():.4f}, max={h_w2g.max():.4f}, NaN={np.isnan(h_w2g).any()}")

print("Applying SGD update to dw2...")
lib.apply_sgd_update(dw2, d_w_conv2_grad, c_float(LR), C2_OUT*C2_IN*KH2*KW2)
h_dw2 = np.zeros(C2_OUT*C2_IN*KH2*KW2, dtype=np.float32)
lib.gpu_memcpy_d2h(h_dw2.ctypes.data, dw2, C2_OUT*C2_IN*KH2*KW2*4)
print(f"dw2 after update: min={h_dw2.min():.4f}, max={h_dw2.max():.4f}, NaN={np.isnan(h_dw2).any()}")
if np.isnan(h_dw2).any() or np.isinf(h_dw2).any():
    print("  BUG: dw2 has NaN/Inf after SGD!")

print("\n=== BACKWARD COMPLETE ===")

# Free
lib.gpu_free(dx)
lib.gpu_free(dc1r)
lib.gpu_free(dc1)
lib.gpu_free(dp1)
lib.gpu_free(dmi1)
lib.gpu_free(dc1_col)
lib.gpu_free(dc2r)
lib.gpu_free(dc2)
lib.gpu_free(dp2)
lib.gpu_free(dmi2)
lib.gpu_free(dc2_col)
lib.gpu_free(dfi)
lib.gpu_free(dfo)
lib.gpu_free(d_fc_grad_w)
lib.gpu_free(d_pool2_grad)
lib.gpu_free(d_conv2_grad)
lib.gpu_free(d_w_conv2_grad)
lib.gpu_free(dw1)
lib.gpu_free(dw2)
lib.gpu_free(dfw)
lib.gpu_free(dfb)
