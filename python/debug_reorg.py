...
print("GEMM output stats:", "min", h_conv_raw.min(), "max", h_conv_raw.max())

lib.leaky_relu_forward(d_conv_raw, c_float(0.1), OC * BATCH * outH * outW)
lib.gpu_memcpy_d2h(h_conv_raw.ctypes.data, d_conv_raw, OC * BATCH * outH * outW * 4)
print("After LeakyReLU stats:", "min", h_conv_raw.min(), "max", h_conv_raw.max())
...
print("After reorganize stats:", "min", h_conv.min(), "max", h_conv.max())
