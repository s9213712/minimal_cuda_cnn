#!/usr/bin/env python3
"""V4: Full training with optimizations - WSL
...
PATIENCE = 3
WD_CONV = 1e-4  # weight decay for conv
WD_FC = 1e-4    # weight decay for fc
...
def augment(x_batch):
    ...
    for i in range(batch_size):
        ...
        # Random crop (pad 4 pixels, crop back to 32)
        pad = 4
        img = x_aug[i]  # (C, H, W)
        # Pad
        ...
        x_aug[i] = padded[:, start_h:start_h + H, start_w:start_w + W]
    return x_aug
...
for i in range(1, 6):
    with open(os.path.join(data_root, f"data_batch_{i}"), "rb") as f:
        ...
        imgs = b[b"data"].astype(np.float32)/255.0
        imgs = imgs.reshape(-1, 3, 32, 32)
        x_train.append(imgs)
        y_train.extend(b[b"labels"])
x_all = np.concatenate(x_train)
y_all = np.array(y_train)
print(f"Data: {x_all.shape}, Train size: {len(y_all)}")

# Split into train (40000) + val (10000)
np.random.seed(42)
...
print(f"\nArch: Conv1({C1_IN}→{C1_OUT})→Pool1→Conv2({C2_IN}→{C2_OUT})→Pool2→FC({FC_IN}→10)")
print(f"LR_conv1={LR_CONV1}, LR_conv2={LR_CONV}, LR_fc={LR_FC}")
print(f"Weight Decay: conv={WD_CONV}, fc={WD_FC}")
print(f"Dropout: {DROPOUT}, Early Stopping: patience={PATIENCE}")
...
for epoch in range(EPOCHS):
    ...
    for batch_idx in range(NBATCHES):
        ...
        x = x_train[indices[idx_s:idx_e]].copy()
        ...
        h_pool2_drop = h_pool2 * mask
        d_fc_in_drop = lib.gpu_malloc(BATCH*FC_IN*4)
        lib.gpu_memcpy_h2d(d_fc_in_drop, h_pool2_drop.ctypes.data, BATCH*FC_IN*4)
        
        d_fc_out = lib.gpu_malloc(BATCH*10*4)
        lib.dense_forward(d_fc_in_drop, d_fc_w, d_fc_b, d_fc_out, BATCH, FC_IN, 10)
        ...
        # FC SGD + Weight Decay
        grad_fc_w = d_loss.T @ h_pool2_drop
        # Weight decay: add L2 penalty (fc_w shape: (23040,) = (10,2304).flatten())
        grad_fc_w_flat = grad_fc_w.flatten()
        grad_fc_w_flat += WD_FC * fc_w
        grad_fc_w = grad_fc_w_flat.reshape(10, FC_IN)
        grad_pool2 = d_loss @ fc_w.reshape(10, FC_IN)
        # Scale by dropout mask for gradient
        grad_pool2 = grad_pool2 * mask
        ...
        # Weight decay on conv2
        grad_w_conv2 = np.frombuffer(bytes(g2h(d_w_conv2_grad, C2_OUT*C2_IN*KH2*KW2)), dtype=np.float32).copy()
        grad_w_conv2 += WD_CONV * w_conv2
        grad_w_conv2 = np.clip(grad_w_conv2, -1.0, 1.0).astype(np.float32)
        lib.gpu_memcpy_h2d(d_w_conv2_grad, grad_w_conv2.ctypes.data, C2_OUT*C2_IN*KH2*KW2*4)
        ...
        # Weight decay on conv1
        grad_w_conv1 = np.frombuffer(bytes(g2h(d_w_conv1_grad, C1_OUT*C1_IN*KH1*KW1)), dtype=np.float32).copy()
        grad_w_conv1 += WD_CONV * w_conv1
        grad_w_conv1 = np.clip(grad_w_conv1, -1.0, 1.0).astype(np.float32)
        lib.gpu_memcpy_h2d(d_w_conv1_grad, grad_w_conv1.ctypes.data, C1_OUT*C1_IN*KH1*KW1*4)
        ...
        lib.gpu_free(d_fc_in_drop); lib.gpu_free(d_fc_out); lib.gpu_free(d_fc_grad_w); lib.gpu_free(d_pool2_grad)
    ...
    # === VALIDATION ===
    val_correct = 0; val_batches = x_val.shape[0] // BATCH
    for vb in range(val_batches):
        vx = x_val[vb*BATCH:(vb+1)*BATCH]
        vy = y_val[vb*BATCH:(vb+1)*BATCH]
        vd_x = lib.gpu_malloc(BATCH*C1_IN*H*W*4)
        lib.gpu_memcpy_h2d(vd_x, vx.ctypes.data, BATCH*C1_IN*H*W*4)
    ...
    print(f"Epoch {epoch+1}: Loss={avg_loss:.4f}, Train={train_acc:.2f}%, Val={val_acc:.2f}%, Time={time.time()-t0:.1f}s")
    ...
    if val_acc > best_val_acc:
        ...
        best_epoch = epoch + 1
        print(f"  ★ New best val_acc={val_acc:.2f}% at epoch {epoch+1}")
    else:
        patience_counter += 1
        ...
        if patience_counter >= PATIENCE:
            ...
            if best_w_conv1 is not None:
                lib.gpu_memcpy_h2d(d_w_conv1, best_w_conv1.ctypes.data, C1_OUT*C1_IN*KH1*KW1*4)
                lib.gpu_memcpy_h2d(d_w_conv2, best_w_conv2.ctypes.data, C2_OUT*C2_IN*KH2*KW2*4)
                lib.gpu_memcpy_h2d(d_fc_w, best_fc_w.ctypes.data, 10*FC_IN*4)
                lib.gpu_memcpy_h2d(d_fc_b, best_fc_b.ctypes.data, 10*4)
...
print("Done! V4 with Early Stopping + Weight Decay + Dropout + Data Augmentation")
