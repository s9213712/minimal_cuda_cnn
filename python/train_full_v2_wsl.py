#!/usr/bin/env python3
"""Full training with conv backward - WSL version"""
import ctypes
import numpy as np
...
print(f"Data: {x_all.shape}, W_conv1: {w_conv1.shape}, W_conv2: {w_conv2.shape}, FC: {fc_w.shape}")
print(f"Arch: Conv1({C1_IN}→{C1_OUT}, {KH1}x{KW1})→Pool({poolH1}x{poolW1})→Conv2({C2_IN}→{C2_OUT})→Pool({poolH2}x{poolW2})→FC({FC_IN}→10)")
print(f"LR_conv={LR_CONV}, LR_fc={LR}, BATCH={BATCH}")
...
for epoch in range(EPOCHS):
    ...
    for batch_idx in range(NBATCHES):
        ...
        # Reorganize backward: conv2_grad_raw (N,OC,H,W) → conv2_grad (OC,N,H,W)
        ...
        h_w_conv2_clipped = np.clip(h_w_conv2, -2.0, 2.0).astype(np.float32)
        lib.gpu_memcpy_h2d(d_w_conv2, h_w_conv2_clipped.ctypes.data, C2_OUT * C2_IN * KH2 * KW2 * 4)
        w_conv2 = h_w_conv2_clipped.copy()
        
        # Pool1 backward (need conv2 grad → pool1 grad)
        # We need d_col2_grad for conv_backward input grad
        # Actually, conv_backward computes weight grad AND input grad
        # We need to also compute d_pool1 from conv2 backward
        # This requires im2col_backward + gemm_backward
        # SKIP for now - conv2 weights alone should still learn something
