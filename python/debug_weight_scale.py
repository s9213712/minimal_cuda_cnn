...
# Try SMALL weights
np.random.seed(42)
w_small = np.random.randn(OC * C * KH * KW).astype(np.float32) * 0.01
# And LARGE weights
w_large = np.random.randn(OC * C * KH * KW).astype(np.float32) * 0.1

for name, w in [("small (0.01)", w_small), ("large (0.1)", w_large)]:
    print(f"\n=== {name} ===")
    print("Weights: min", w.min(), "max", w.max())
    ...
    print("Conv: min", h_conv.min(), "max", h_conv.max())
