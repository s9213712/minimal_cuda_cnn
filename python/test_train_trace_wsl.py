#!/usr/bin/env python3
"""Full training loop trace - run many batches, detect first corruption"""
import ctypes
import numpy as np
...
def check(name, arr):
    ...
    bad = nan + inf
    print(f"    {name}: {arr.min():.4f}/{arr.max():.4f}, nan={nan}, inf={inf} {'❌' if bad else '✅'}")
    return bad == 0
...
for batch_idx in range(MAX_BATCHES):
    ...
    ok = check(f"B{batch_idx} pool2", h_pool2)
    if not ok:
        print(f"  ❌ BREAK at batch {batch_idx} - pool2 corrupted!")
        print(f"  h_pool2 sample: {h_pool2[0, :10]}")
        break
    ...
    ok = check(f"B{batch_idx} fc_out", h_out)
    if not ok:
        print(f"  ❌ BREAK at batch {batch_idx} - fc_out corrupted!")
        break
    ...
    lib.gpu_memcpy_h2d(d_pool2_grad, np.clip(grad_pool2, -1.0, 1.0).flatten().astype(np.float32).ctypes.data, C2_OUT * BATCH * poolH2 * poolW2 * 4)
    ...
    if not check(f"B{batch_idx} w_conv1", h_w1) or not check(f"B{batch_idx} w_conv2", h_w2) or not check(f"B{batch_idx} fc_w", h_fc):
        print(f"  ❌ BREAK at batch {batch_idx} - weights corrupted!")
        break
...
print(f"\nDone. Processed {batch_idx+1} batches.")
