#!/usr/bin/env python3
"""Best version: Conv1 frozen, Conv2+FC updating - WSL"""
import ctypes
import numpy as np
...
print(f"Arch: Conv1({C1_IN}→{C1_OUT}, frozen) → Pool1 → Conv2({C2_IN}→{C2_OUT}) → Pool2 → FC({FC_IN}→10)")
print(f"LR_conv={LR_CONV}, LR_fc={LR_FC}, BATCH={BATCH}, EPOCHS={EPOCHS}")
print(f"NOTE: Conv1 FROZEN (gradient broken), Conv2+FC updating")
print()
...
print("\nDone! Conv1 was frozen (gradient bug not fixed yet)")
