#!/usr/bin/env python3
"""Channel-by-channel pool analysis"""
import ctypes
import numpy as np
...
# Examine by channel (each channel is 30*30 = 900 elements)
print("Per-channel max in batch 0:")
for c in range(OC):
    start = c * outH * outW
    ch_data = h_conv[0, start:start+outH*outW]
    print(f"  Ch {c}: min={ch_data.min():.4f}, max={ch_data.max():.4f}, nonzero={np.count_nonzero(ch_data)}")
