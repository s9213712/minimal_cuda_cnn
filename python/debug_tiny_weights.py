#!/usr/bin/env python3
"""Debug im2col to find the overflow source"""
import ctypes
import numpy as np
...
# Use tiny weights
np.random.seed(42)
w = np.random.randn(OC * C * KH * KW).astype(np.float32) * 0.001
...
print("im2col:", h_col.shape, "min", h_col.min(), "max", h_col.max())
...
print("GEMM:", h_conv.shape, "min", h_conv.min(), "max", h_conv.max())
