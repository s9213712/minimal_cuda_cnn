...
def run(lr_conv1, desc):
    ...
    for epoch in range(EPOCHS):
        for batch_idx in range(NBATCHES):
            ...
            # Conv1 backward with custom lr_conv1
...
print(f"LR_CONV2={LR_CONV2}, LR_FC={LR_FC}, EPOCHS={EPOCHS}")
for lr1 in [0.0, 0.0001, 0.0005, 0.001, 0.005]:
    t0 = time.time()
    acc = run(lr1, f"LR1={lr1}")
    print(f"  LR_conv1={lr1}: acc={acc:.2f}%, time={time.time()-t0:.0f}s")
