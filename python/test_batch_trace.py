#!/usr/bin/env python3
"""Track where NaN appears in training loop"""
import ctypes
import numpy as np
...
with open(os.path.join(data_root, "data_batch_1"), "rb") as f:
    ...
    x_all = batch[b"data"][:128].astype(np.float32) / 255.0
    y_all = np.array(batch[b"labels"][:128])
    x_all = x_all.reshape(-1, 3, 32, 32)
print(f"Data: {x_all.shape}")

for batch_idx in range(2):
    x = x_all[batch_idx*64:(batch_idx+1)*64]
    y = y_all[batch_idx*64:(batch_idx+1)*64]
    print(f"\n=== Batch {batch_idx} ===")
    ...
    lib.im2col_forward(d_x, dc1_col, BATCH, C1_IN, H, W, KH1, KW1, outH1, outW1)
    lib.gemm_forward(d_w_conv1, dc1_col, dc1r, C1_OUT, BATCH*outH1*outW1, C1_IN*KH1*KW1)
    ...
    lib.gemm_forward(d_w_conv2, dc2_col, dc2r, C2_OUT, BATCH*outH2*outW2, C2_IN*KH2*KW2)
    ...
    lib.dense_forward(dfi, d_fc_w, d_fc_b, dfo, BATCH, FC_IN, 10)
    
    h_out = np.zeros((BATCH, 10), dtype=np.float32)
    lib.gpu_memcpy_d2h(h_out.ctypes.data, dfo, BATCH*10*4)
    
    print(f"h_out: min={h_out.min():.4f}, max={h_out.max():.4f}, NaN={np.isnan(h_out).any()}")
    
    h_sh = h_out - h_out.max(axis=1, keepdims=True)
    exp_o = np.exp(h_sh)
    probs = exp_o / exp_o.sum(axis=1, keepdims=True)
    print(f"probs: min={probs.min():.4f}, max={probs.max():.4f}, NaN={np.isnan(probs).any()}")
    ...
    dc2_grad = lib.gpu_malloc(C2_OUT*BATCH*outH2*outW2*4)
    lib.gpu_memset(dc2_grad, 0, C2_OUT*BATCH*outH2*outW2*4)
    lib.maxpool_backward_use_idx(d_pool2_grad, dmi2, dc2_grad, BATCH, C2_OUT, outH2, outW2)
    
    dw2_grad = lib.gpu_malloc(C2_OUT*C2_IN*KH2*KW2*4)
    lib.gpu_memset(dw2_grad, 0, C2_OUT*C2_IN*KH2*KW2*4)
    lib.conv_backward(dc2_grad, dp1, d_w_conv2, dw2_grad,
                      BATCH, C2_IN, poolH1, poolW1, KH2, KW2, outH2, outW2, C2_OUT)
    lib.apply_sgd_update(d_w_conv2, dw2_grad, c_float(LR), C2_OUT*C2_IN*KH2*KW2)
    
    # Check weights after update
    h_dw2 = np.zeros(C2_OUT*C2_IN*KH2*KW2, dtype=np.float32)
    lib.gpu_memcpy_d2h(h_dw2.ctypes.data, d_w_conv2, C2_OUT*C2_IN*KH2*KW2*4)
    ...
    lib.gpu_free(dc2_grad)
    lib.gpu_free(dw2_grad)
