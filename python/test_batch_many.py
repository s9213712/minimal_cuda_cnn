#!/usr/bin/env python3
"""Track many batches to find when NaN appears"""
import ctypes
import numpy as np
...
with open(os.path.join(data_root, "data_batch_1"), "rb") as f:
    ...
    x_all = batch[b"data"][:640].astype(np.float32) / 255.0
    y_all = np.array(batch[b"labels"][:640])
...
for batch_idx in range(NBATCHES):
    ...
    if np.isnan(h_out).any():
        print(f"Batch {batch_idx}: h_out NaN!")
        break
    if np.isinf(h_out).any():
        print(f"Batch {batch_idx}: h_out Inf!")
        break
    ...
    print(f"Batch {batch_idx}: Loss={loss:.4f}, Acc={acc}/{BATCH}")
