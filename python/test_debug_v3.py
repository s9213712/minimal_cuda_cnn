...
NBATCHES = 5
print("Starting 5-batch test with persistent buffers...")
...
for batch_idx in range(NBATCHES):
    ...
    if batch_idx == 0:
        print(f"Batch {batch_idx+1}: loss={loss:.4f}, probs sum={probs.sum():.2f}, max={probs.max():.4f}")
        # Check dense_forward output
        print(f"  h_out sample: {h_out[0,:5]}")
        print(f"  h_pool2 sample: {h_pool2[0,:5]}")
    ...
    if batch_idx == 0:
        ...
        print(f"  d_din: any_nan={np.any(np.isnan(h_din))}, max={h_din.max():.4f}")
        h_dw = g2h(d_fc_grad_w, 10*FC_IN)
        print(f"  d_fc_grad_w: any_nan={np.any(np.isnan(h_dw))}, max={h_dw.max():.4f}")
    ...
    if batch_idx == 0:
        print(f"Batch {batch_idx+1} complete: loss={loss:.4f}")

print("5 batches done!")
