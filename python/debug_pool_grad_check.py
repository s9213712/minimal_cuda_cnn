#!/usr/bin/env python3
"""Check which locations get gradient in pool backward"""
import ctypes
import numpy as np
...
# Check conv values at first position
...
print("Checking pool backward with grad=1 for first output position (oh=0, ow=0)")
print("First channel, first output position")

# Pool output at (0, 0, 0, 0) = max of conv[0,0,0:2,0:2]
window = h_conv[0, 0, 0:2, 0:2]
print(f"Conv window [0:2, 0:2]:\n{window}")
print(f"Max in window: {window.max():.6f} at row={window.argmax()//2}, col={window.argmax()%2}")
print(f"Pool output at (0,0,0,0): {h_pool[0,0,0,0]:.6f}")

# Now test backward - set grad=1 only at first pool output
grad_out_data = np.zeros((BATCH, OC, 15, 15), dtype=np.float32)
grad_out_data[0, 0, 0, 0] = 1.0
...
print(f"\nConv grad in first 2x2 window:")
print(h_conv_grad[0, 0, 0:2, 0:2])
print(f"Sum: {h_conv_grad[0, 0, 0:2, 0:2].sum():.6f}")
