#!/usr/bin/env python3
"""Quick test for weight clipping"""
import ctypes
import numpy as np
...
print(f"h_p2: min={h_p2.min():.4f}, max={h_p2.max():.4f}")
...
print("FC update done")

h_fc_w = np.zeros(10 * FC_IN, dtype=np.float32)
lib.gpu_memcpy_d2h(h_fc_w.ctypes.data, dfw, 10 * FC_IN * 4)
print(f"h_fc_w after update: min={h_fc_w.min():.4f}, max={h_fc_w.max():.4f}")
h_fc_w_c = np.clip(h_fc_w, -1.0, 1.0).astype(np.float32)
print(f"h_fc_w clipped: min={h_fc_w_c.min():.4f}, max={h_fc_w_c.max():.4f}")
lib.gpu_memcpy_h2d(dfw, h_fc_w_c.ctypes.data, 10 * FC_IN * 4)
fc_w = h_fc_w_c.copy()

fc_w_r = fc_w.reshape(10, FC_IN)
gp2 = d_loss @ fc_w_r
gp2_c = np.clip(gp2, -1.0, 1.0).astype(np.float32)
dp2g = lib.gpu_malloc(C2_OUT * BATCH * poolH2 * poolW2 * 4)
lib.gpu_memcpy_h2d(dp2g, gp2_c.flatten().ctypes.data, C2_OUT * BATCH * poolH2 * poolW2 * 4)

dc2g = lib.gpu_malloc(C2_OUT * BATCH * outH2 * outW2 * 4)
lib.gpu_memset(dc2g, 0, C2_OUT * BATCH * outH2 * outW2 * 4)
lib.maxpool_backward_use_idx(dp2g, dmi2, dc2g, BATCH, C2_OUT, outH2, outW2)
print("Pool2 backward done")
...
print("Conv2 backward done")

lib.apply_sgd_update(dw2, dw2g, c_float(LR), C2_OUT * C2_IN * KH2 * KW2)
print("Conv2 weight update done")

h_dw2 = np.zeros(C2_OUT * C2_IN * KH2 * KW2, dtype=np.float32)
lib.gpu_memcpy_d2h(h_dw2.ctypes.data, dw2, C2_OUT * C2_IN * KH2 * KW2 * 4)
print(f"h_dw2 after update: min={h_dw2.min():.4f}, max={h_dw2.max():.4f}")
h_dw2_c = np.clip(h_dw2, -1.0, 1.0).astype(np.float32)
lib.gpu_memcpy_h2d(dw2, h_dw2_c.ctypes.data, C2_OUT * C2_IN * KH2 * KW2 * 4)
w2 = h_dw2_c.copy()
