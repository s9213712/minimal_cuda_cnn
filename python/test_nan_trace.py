#!/usr/bin/env python3
"""Check for NaN after each step in first 3 batches"""
...
lib.dense_forward.argtypes = [c_void_p]*4 + [c_int]*3
...
BATCH=64; H=32; W=32; KH1=3; KW1=3; C1_IN=3; C1_OUT=32
outH1=30; outW1=30; poolH1=15; poolW1=15; C2_IN=32; C2_OUT=64
KH2=3; KW2=3; outH2=13; outW2=13; poolH2=6; poolW2=6
FC_IN=64*6*6; LR_CONV=0.01; LR_CONV1=0.001; LR_FC=0.01
...
def check(name, arr):
    n = np.any(np.isnan(arr)); inf = np.any(np.isinf(arr))
    if n or inf: print(f"  {name}: NaN={n}, Inf={inf}, min={arr.min():.4f}, max={arr.max():.4f}")
    return n or inf

for batch_idx in range(3):
    x = x_all[batch_idx*BATCH:(batch_idx+1)*BATCH]
    y = y_all[batch_idx*BATCH:(batch_idx+1)*BATCH]
    ...
    check("conv1_raw", g2h(d_conv1_raw, C1_OUT*BATCH*outH1*outW1))
    ...
    check("conv2_raw", g2h(d_conv2_raw, C2_OUT*BATCH*outH2*outW2))
    ...
    if check("pool2", h_pool2): print(f"    Batch {batch_idx+1} FAIL at pool2"); break
    ...
    if check("fc_out", h_out): print(f"    Batch {batch_idx+1} FAIL at fc_out"); break
    ...
    h_din = g2h(d_din, BATCH*FC_IN)
    if check("d_din", h_din): print(f"    Batch {batch_idx+1} FAIL at d_din"); break
    h_dw = g2h(d_fc_grad_w, 10*FC_IN)
    if check("d_fc_grad_w", h_dw): print(f"    Batch {batch_idx+1} FAIL at d_fc_grad_w"); break
    ...
    if check("fc_w after update", h_fc_w): print(f"    Batch {batch_idx+1} FAIL at fc_w update"); break
    ...
    print(f"Batch {batch_idx+1}: loss={loss:.4f}, acc={np.mean(np.argmax(probs,1)==y)*100:.1f}%")

print("3 batches done!")
