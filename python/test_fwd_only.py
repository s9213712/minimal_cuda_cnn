#!/usr/bin/env python3
"""Forward-only test to check if explosion is in forward"""
import ctypes
import numpy as np
...
print(f"fc_w: min={fc_w.min():.6f}, max={fc_w.max():.6f}, mean={fc_w.mean():.6f}")
...
# Check GPU weights after copy
h_w1_check = np.zeros(C1_OUT * C1_IN * KH1 * KW1, dtype=np.float32)
h_w2_check = np.zeros(C2_OUT * C2_IN * KH2 * KW2, dtype=np.float32)
lib.gpu_memcpy_d2h(h_w1_check.ctypes.data, dw1, C1_OUT * C1_IN * KH1 * KW1 * 4)
lib.gpu_memcpy_d2h(h_w2_check.ctypes.data, dw2, C2_OUT * C2_IN * KH2 * KW2 * 4)
print(f"GPU w1: min={h_w1_check.min():.6f}, max={h_w1_check.max():.6f}")
print(f"GPU w2: min={h_w2_check.min():.6f}, max={h_w2_check.max():.6f}")
...
# Check im2col output
h_col1 = np.zeros(C1_IN * KH1 * KW1 * BATCH * outH1 * outW1, dtype=np.float32)
lib.gpu_memcpy_d2h(h_col1.ctypes.data, dc1_col, C1_IN * KH1 * KW1 * BATCH * outH1 * outW1 * 4)
print(f"im2col col1: min={h_col1.min():.4f}, max={h_col1.max():.4f}")

lib.gemm_forward(dw1, dc1_col, dc1r, C1_OUT, BATCH * outH1 * outW1, C1_IN * KH1 * KW1)

# Check conv1 raw output
h_conv1_raw = np.zeros(C1_OUT * BATCH * outH1 * outW1, dtype=np.float32)
lib.gpu_memcpy_d2h(h_conv1_raw.ctypes.data, dc1r, C1_OUT * BATCH * outH1 * outW1 * 4)
print(f"Conv1 raw: min={h_conv1_raw.min():.4f}, max={h_conv1_raw.max():.4f}")

lib.leaky_relu_forward(dc1r, c_float(0.1), C1_OUT * BATCH * outH1 * outW1)

# Check after leaky ReLU
h_conv1_relu = np.zeros(C1_OUT * BATCH * outH1 * outW1, dtype=np.float32)
lib.gpu_memcpy_d2h(h_conv1_relu.ctypes.data, dc1r, C1_OUT * BATCH * outH1 * outW1 * 4)
print(f"Conv1 ReLU: min={h_conv1_relu.min():.4f}, max={h_conv1_relu.max():.4f}")
...
# Check pool1
h_pool1 = np.zeros(C1_OUT * BATCH * poolH1 * poolW1, dtype=np.float32)
lib.gpu_memcpy_d2h(h_pool1.ctypes.data, dp1, C1_OUT * BATCH * poolH1 * poolW1 * 4)
print(f"Pool1: min={h_pool1.min():.4f}, max={h_pool1.max():.4f}")
...
h_col2 = np.zeros(C2_IN * KH2 * KW2 * BATCH * outH2 * outW2, dtype=np.float32)
lib.gpu_memcpy_d2h(h_col2.ctypes.data, dc2_col, C2_IN * KH2 * KW2 * BATCH * outH2 * outW2 * 4)
print(f"im2col col2: min={h_col2.min():.4f}, max={h_col2.max():.4f}")

lib.gemm_forward(dw2, dc2_col, dc2r, C2_OUT, BATCH * outH2 * outW2, C2_IN * KH2 * KW2)

h_conv2_raw = np.zeros(C2_OUT * BATCH * outH2 * outW2, dtype=np.float32)
lib.gpu_memcpy_d2h(h_conv2_raw.ctypes.data, dc2r, C2_OUT * BATCH * outH2 * outW2 * 4)
print(f"Conv2 raw: min={h_conv2_raw.min():.4f}, max={h_conv2_raw.max():.4f}")

lib.leaky_relu_forward(dc2r, c_float(0.1), C2_OUT * BATCH * outH2 * outW2)

h_conv2_relu = np.zeros(C2_OUT * BATCH * outH2 * outW2, dtype=np.float32)
lib.gpu_memcpy_d2h(h_conv2_relu.ctypes.data, dc2r, C2_OUT * BATCH * outH2 * outW2 * 4)
print(f"Conv2 ReLU: min={h_conv2_relu.min():.4f}, max={h_conv2_relu.max():.4f}")
...
h_pool2 = np.zeros(C2_OUT * BATCH * poolH2 * poolW2, dtype=np.float32)
lib.gpu_memcpy_d2h(h_pool2.ctypes.data, dp2, C2_OUT * BATCH * poolH2 * poolW2 * 4)
print(f"Pool2: min={h_pool2.min():.4f}, max={h_pool2.max():.4f}")

# FC
h_pool2_flat = h_pool2.reshape(BATCH, -1)
dfi = lib.gpu_malloc(BATCH * FC_IN * 4)
lib.gpu_memcpy_h2d(dfi, h_pool2_flat.ctypes.data, BATCH * FC_IN * 4)
