...
fc_w = h_fc_w_c.copy()
print(f"fc_w after SGD: min={fc_w.min():.4f} max={fc_w.max():.4f}")

# SECOND FORWARD (to check if FC update corrupted anything)
lib.gpu_memcpy_h2d(dfi, h_pool2.ctypes.data, BATCH * FC_IN * 4)
lib.dense_forward(dfi, dfw, dfb, dfo, BATCH, FC_IN, 10)
...
print("dc2g memset done")
lib.maxpool_backward_use_idx(dp2g, dmi2, dc2g, BATCH, C2_OUT, outH2, outW2)
print("maxpool_backward_use_idx done")

# THIRD FORWARD (after pool2 backward)
# Re-run conv layers
lib.im2col_forward(dp1, dc2_col, BATCH, C2_IN, poolH1, poolW1, KH2, KW2, outH2, outW2)
lib.gemm_forward(dw2, dc2_col, dc2r, C2_OUT, BATCH * outH2 * outW2, C2_IN * KH2 * KW2)
lib.leaky_relu_forward(dc2r, c_float(0.1), C2_OUT * BATCH * outH2 * outW2)
lib.reorganize_forward(dc2r, dc2, BATCH, C2_OUT, outH2, outW2)
lib.maxpool_forward_store(dp2, dc2, dmi2, BATCH, C2_OUT, outH2, outW2)
h_pool2_3 = np.zeros((BATCH, FC_IN), dtype=np.float32)
lib.gpu_memcpy_d2h(h_pool2_3.ctypes.data, dp2, BATCH * FC_IN * 4)
lib.gpu_memcpy_h2d(dfi, h_pool2_3.ctypes.data, BATCH * FC_IN * 4)
lib.dense_forward(dfi, dfw, dfb, dfo, BATCH, FC_IN, 10)
h_out3 = np.zeros((BATCH, 10), dtype=np.float32)
lib.gpu_memcpy_d2h(h_out3.ctypes.data, dfo, BATCH * 10 * 4)
print(f"Third forward h_out3: min={h_out3.min():.4f} max={h_out3.max():.4f}")
...
lib.gpu_free(dfo); lib.gpu_free(dgfw); lib.gpu_free(dp2g); lib.gpu_free(dc2g)
